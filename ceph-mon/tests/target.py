...
def setup_osds():
    ...
    for i in range(3):
        file = 'loop-%d' % i
        unit = 'ceph-osd/%d' % i
        zaza_model.run_on_unit(unit, 'touch %s' % file)
        zaza_model.run_on_unit(unit, 'truncate --size 2G ./%s' % file)
        out = zaza_model.run_on_unit(unit,
                                     'sudo losetup -fP --show ./%s' % file)

        loop = out['Stdout'].strip()
        zaza_model.run_action(unit_name=unit, action_name='add-disk',
                              action_params={'osd-devices': loop})

    zaza_model.wait_for_application_states(states=None)
